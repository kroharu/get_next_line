...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/07 16:59:05 by ladrian           #+#    #+#             */
/*   Updated: 2021/12/07 17:46:38 by ladrian          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > OPEN_MAX - 1)
		return (0);
	static_line[fd] = ft_read_line_and_save(fd, static_line[fd]);
	if (!static_line[fd])
		return (NULL);
	line = ft_get_line(static_line[fd]);
	static_line[fd] = ft_save_line(static_line[fd]);
	return (line);
}
